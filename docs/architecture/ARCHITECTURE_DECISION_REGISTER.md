...
| ADR-016 | Hosting | ~~**Open** — Vercel vs a single container on a managed host~~ **Decided by ADR-046 (2026-09-26): one DigitalOcean Droplet, container image, Caddy** | Self-managed VM/Kubernetes (rejected as oversized) | Evidence-pack duration limits and residency drive the choice | Ops, cost, residency | 🔴 Yes | **RESOLVED by ADR-046** 2026-09-26 — provisioning pending |
...
| **ADR-045** | **Repository layout and the mockup's relationship to production** | **Production application at the repository root** (as the accepted Milestone 1 scope already stated); `project-artifacts/mockup/` **kept, untouched, for comparison**; mockup material crosses to production **only by a reviewed port from an explicit PORT list**, never by copy; fixture data confined to a lint-fenced `src/preview/` | Sibling folder (`project-artifacts/app`, `apps/portal` + monorepo tooling — new technology); copy the mockup wholesale (carries simulations and static-export config); delete the mockup on cutover (destructive, refused) | Closes external-review finding **I-3** (Track A medium) and **A-2** (fixture contamination); one application, one deploy (ADR-001) | Every milestone; the mockup's lifecycle | 🔴 Yes | **APPROVED** 2026-09-21 — founder direction; see record |
| **ADR-046** | **Production hosting, deployment governance and Stripe key type (Milestone 11 K1–K16)** | **DigitalOcean**: one Droplet (Ubuntu 24.04, 2 GiB/2 vCPU, SGP1) running the CI-built container image behind **Caddy**, **Managed PostgreSQL 16** in the same VPC, **staging on the same Droplet**; images built and proven only by CI on a `v*` tag; the governed `deploy/` framework (signed token, root wrapper, backup + sandbox before every migration, auto-rollback, audit); release gate adopted with Playwright blocking; managed PITR + nightly dump + weekly off-host copy; systemd timer for reminders; **Stripe restricted keys (`rk_`) accepted** with a documented minimum permission set; one real payment + refund at go-live | Vercel + Neon (M9 Option A); Fly/Railway (Option B); PM2 + nginx + on-box PostgreSQL (eCard parity); DO App Platform; Colima builds on the laptop; no staging; nginx; a lighter SSH+compose pipeline without a token | Resolves ADR-016, ADR-005a (production host), OQ-18/J6; keeps ADR-029 (staging, forward-only migrations) and ADR-030 (secrets only on the host); eCard's proven controls on the portal's own runtime model — `MILESTONE_11_EXECUTION_PLAN.md` §3 | Production infrastructure, release process, payments configuration | 🔴 Yes | **APPROVED** 2026-09-26 — founder: *"K1 = yes, all recommendations accepted, start Phase A"*; see record. **Provisioning (Phase B) not yet done** |
| **ADR-047** | **Trainer role and trainings managed in the portal (Milestone 12 L1–L12)** | The `expert` role IS the Trainer role (labelled "Trainer"); Trainers work inside `/admin` on a reduced surface, scoped to trainings linked through `programme_experts`; only administrators publish; **four fee rows per training** — `price_region` gains `malaysia_hrdcorp`, `programme_prices` gains `min_participants` and `note` (additive migration); the HRD Corp row is display-only, a Malaysian card payment is charged the not-via-HRD-Corp row; the seed becomes an initial import (insert-if-missing) and the database is the truth; every editorial section editable; a Trainer profile is created on grant; the training page lists its open dates; "International" → "Rest of the world" | A new `trainer` enum value; a separate `/trainer` area; Trainers publishing directly; an HRD Corp claim checkout now; keeping the seed authoritative; a launch-critical subset of sections first | Resolves M8 §5 A7 (trainer role) and PROJECT_STATUS item 8(a); implements the founder's 2026-09-26 request "at database level, nothing at front end" without inventing business rules | Catalogue, identity/roles, admin area, public pricing and schedule, seed policy | 🔴 Yes (schema, authorisation) | **APPROVED** 2026-09-26 — founder: *"All recommendations accepted, L1-L12 = yes, start M12"*; built the same day (`MILESTONE_12_EXECUTION_PLAN.md` §8) |
...
### ADR-045 — Repository layout and the mockup's relationship to production
**Date:** 2026-09-21 · **Status:** APPROVED — 2026-09-21 · **Closes:** external review findings **I-3** (Track A medium) and **A-2** (fixture contamination) · **Arises from:** `docs/execution/WIREFRAME_TO_PRODUCTION_PLAN.md` §3–§4

**Context.** The wireframe in `project-artifacts/mockup/` grew to 83 pages and became the only working form of the product's design system, copy and real content — while remaining, by its own README, *"a disposable, isolated artifact … nothing carries forward automatically."* The accepted Milestone 1 scope placed the production application at the repository root, but no ADR recorded that, and nothing anywhere ruled on *what*, if anything, crosses from the mockup to production. The external review named this gap "the largest genuine hole in the Phase 1 plan."

**Decision.**

> **The production application lives at the repository root** (`app/`, `src/`, `prisma/`, `tests/`, `package.json`). **`project-artifacts/mockup/` is retained, unmodified, for comparison** until an explicit, separate instruction retires it. **Mockup material reaches production only through a reviewed port** — file by file, with a provenance comment naming the source and date — **and only if it is on the PORT list**; the NEVER-PORT list is deleted with the mockup. **Fixture or preview data lives only under `src/preview/`, and a lint rule forbids any production module importing from it.**

PORT (reviewed copy): design tokens and type scale (`globals.css`) · the `ui/` primitives · header/footer/mobile-menu chrome and theme toggle · page structure and founder-reviewed copy of the public pages · content files (`courses`, `practitioners`, `hrdCorp`, `faq`, `questions`) **as seed data** · `lib/certificates.ts` (tested rules), the certificate document and print stylesheet · screen structure of account, checkout, verify and admin.
NEVER PORT: every `demo*` simulation, `sessionStorage` state, sample registries and code-constant fees · `generateStaticParams` workarounds, `output: "export"`, `basePath` · honesty labels (`SampleTag`, `WireframeNote`, watermark, demo banner) · the browser-side YouTube key pattern.

**Alternatives considered.** (a) *A sibling folder* (`project-artifacts/app`, or `apps/portal` with workspace tooling) — reopens an accepted scope decision with no AP-10 question-1 answer, and a monorepo tool is new technology for a single application (ADR-001). (b) *Copy the mockup wholesale and "connect" it* — carries the simulated payment success and identity into production (violates F6, AP-07) and inherits static-hosting assumptions. (c) *Delete the mockup at cutover* — destructive; refused without a separate instruction. (d) *Port nothing* — re-derives 83 reviewed pages from the specifications: waste.

**Consequences.** Two `package.json` files coexist in one repository (root and mockup); the mockup keeps port 3000, the production app takes 3100. Milestone 1's scope gains the `/preview` fence and the ported tokens/primitives (`WIREFRAME_TO_PRODUCTION_PLAN.md` §7 M1). Every later milestone's execution plan lists the mockup files it retires. The mockup is retired at M10 by marking its README, never by deletion (plan §9).

**Approval record.**
- **Approved:** 2026-09-21, by the founder. Instruction, verbatim: *"1. Will there be another folder like mockup for the real implementation? — Yes keep it for comparison at later stage. Rest please go ahead and implement the production ready product."* Recorded as: keep the mockup folder; proceed with the plan as proposed, which places production at the root. ⚠ *If "Yes" was intended to mean "put production in a separate folder", say so — moving the root application into a sibling folder is a mechanical `git mv` and reversible; nothing built today depends on its location.*
- **What is approved:** Root placement; retention of the mockup for comparison; the PORT / NEVER-PORT policy; the `src/preview/` fence.
- **What this approval does NOT authorize:** Moving, renaming or deleting anything under `project-artifacts/mockup/` · any vendor selection · any production infrastructure · any change to the repository's remote or boundaries.

---

### ADR-046 — Production hosting, deployment governance and Stripe key type (Milestone 11)
**Date:** 2026-09-26 · **Status:** APPROVED — 2026-09-26 · **Resolves:** ADR-016 (hosting), ADR-005a (production PostgreSQL host), OQ-18 / M9 J6 (release gate); refines J1, J7, J9 · **Arises from:** `docs/execution/MILESTONE_11_EXECUTION_PLAN.md` §5 (K1–K16)

**Context.** The founder asked for Stripe "as production ready" and for the portal to be deployed on a DigitalOcean server with a deployment framework modelled on the eCard repository's `Deployement-Steps/`. Stripe was already implemented (Milestone 4) and stronger than eCard's; the founder's new key is a *restricted* key (`rk_live_`), which `src/config/env.ts` rejected; hosting, database and release gate were still open from Milestone 9. Sixteen numbered decisions were put with recommendations and a monthly cost (≈ $33–38).

**Decision.**

> **K1** `src/config/env.ts` accepts `rk_live_`/`rk_test_` beside `sk_`; the live key is created with exactly *Checkout Sessions write · Refunds write · PaymentIntents, Charges, Balance transactions read*. **K2 (a)** one DigitalOcean Droplet running the container image (existing `Dockerfile`) behind **Caddy** (K8). **K3** DigitalOcean **Managed PostgreSQL 16** in the same VPC (backups + PITR), never on the Droplet. **K4** Basic 2 GiB / 2 vCPU. **K5** SGP1 — residency inputs (J1) still to be confirmed before production data is provisioned (ADR-032 sequencing rule). **K6** images built, proven and pushed **only by CI** on a `v*` tag (`release.yml`); no container runtime on the laptop, no build on the server. **K7** staging on the same Droplet (separate container, port and database). **K9** full governance port from eCard: HMAC-signed expiring token, root-owned `p4tc-deploy` wrapper as the only sudo entry, lock, manifest, backup before every promotion, migration sandbox on a copy of the database, destructive DDL refused unless approved in git, auto-rollback on failed health, reports and timeouts on every task, no bypass flags. **K10** managed PITR primary + nightly on-server dump (keep 7) + weekly off-host copy by the founder. **K11** `RELEASE_GATE.md` adopted as written, Playwright **blocking**. **K12** systemd timer for the reminders job. **K13** domain — founder's choice, still open. **K14** email stays `EMAIL_TRANSPORT=log` at go-live. **K15** the nine webhook events as documented. **K16** one real registration by the founder at go-live, refunded at once.

**Alternatives considered.** Vercel + Neon and Fly/Railway (M9 Options A/B — a second platform vocabulary the founder does not operate); PM2 + nginx + on-box PostgreSQL (eCard parity — a Next.js build on a small Droplet is slow and non-reproducible, and the database would share the box's fate); DO App Platform (less control over migrations and the sandbox); a lighter SSH + compose pipeline without a token (loses the controls that make a deploy an auditable act); no staging (conflicts with ADR-029).

**Consequences.** `deploy/` becomes part of the product; `Dockerfile` gains a `migrate` target so migrations travel with the tag; `ci.yml` is callable from `release.yml`; `DEPLOYMENT_RUNBOOK.md` Option C supersedes A/B in practice; the Stripe permission set is documented in three places (`.env.example`, runbook §2, MONITORING §4.6). Nothing is provisioned by this decision — every Phase B step (account, Droplet, cluster, registry, DNS, env files, live webhook) is a separate RED action the founder takes; the pasted live key must be rolled first (`MILESTONE_11_EXECUTION_PLAN.md` §0).

**Approval record.**
- **Approved:** 2026-09-26, by the founder. Instruction, verbatim: *"K1 = yes, all recommendations accepted, start Phase A"*.
- **What is approved:** K1–K16 exactly as recommended in the plan's §5; Phase A implementation (code, framework, workflow, docs).
- **What this approval does NOT authorize:** Creating any DigitalOcean resource, registry, DNS record or Stripe live endpoint · placing a live key anywhere but the server's root-owned env file · a production deploy · any change to checkout, pricing, refund or webhook logic · confirming the residency answer (J1) — each remains an explicit founder action.
...
> **L1** The existing `expert` role is the Trainer role, shown as "Trainer"; no new enum value. **L2** Only administrators publish. **L3** Trainers work inside `/admin`, role-aware: a reduced bar and dashboard, every other screen a 403. **L4** Three additive schema changes: `price_region` value `malaysia_hrdcorp`; `programme_prices.min_participants`, `.note`. **L5** The HRD Corp row is display-only ("claim through your employer"); Stripe never charges it. **L6** A Malaysian card payment is charged the not-via-HRD-Corp row. **L7** A Trainer's scope is the trainings linked to their profile through `programme_experts`. **L8** `db:seed` becomes insert-if-missing for trainings, modules, formats and fees — the portal is the truth; the seed files are the initial import. **L9** Every editorial section is editable (plain text fields with documented line formats — no editor dependency). **L10** Granting Trainer creates an unpublished Trainer profile when none exists. **L11** The training page lists its open dates with Register. **L12** "International" is renamed "Rest of the world".

**Alternatives considered.** A new `trainer` enum value (a migration for a label); a separate `/trainer` area (a second UI to maintain); Trainers publishing directly (no human check before a price and a date go public); building the HRD Corp claim/invoice path now (its own milestone — M8 §5 A3); keeping the seed authoritative (admin edits lost on the next seed); a launch-critical subset of sections (would leave "nothing at front end" untrue until a follow-up).

**Consequences.** `/admin/trainings` (six tabs) becomes the only way trainings change; `programmes.content` is written by the content editor alone; the checkout rule for Malaysia is data (`regionForCountry` → `malaysia`), not a JSON convention; the seed can no longer be used to push content changes to an existing database (`db:reset` on a throwaway database re-imports). Supporting change: `/schedule` lists every published training's dates (it listed the flagship's only, so a launched training could not be booked). Not authorised by this ADR: an HRD Corp claim checkout, attendance by trainers, payouts, image upload, or any production deployment.

**Approval record.**
- **Approved:** 2026-09-26, by the founder. Instruction, verbatim: *"All recommendations accepted, L1-L12 = yes, start M12"*.
- **What is approved:** L1–L12 exactly as recommended in the plan's §6, including the schema changes in its §4.
- **What this approval does NOT authorize:** Any further schema change · an HRD Corp registration/invoice workflow · trainer-recorded attendance or payouts · any change to refund, webhook or Stripe logic · deletion of any training (status changes only).
