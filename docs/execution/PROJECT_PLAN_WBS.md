...
| WBS | Work package | Status | Depends on | Blocking decision | Reference | Exit criterion |
|---|---|---|---|---|---|---|
| **4.2.2** | **Milestone 2 — the first real steps of the slice.** Authentication, scoped RBAC enforced at the service and data-access layer, dashboard | ✅ **Executed 2026-09-21** on the ADR-006 *recommendation* (Better Auth) under the founder's blanket direction — **B1/B2/B3 ratification pending** | 4.2.1 complete | **ADR-006 — B1 · B2 · B3** (ratification) | `MILESTONE_2_EXECUTION_PLAN.md`; `MILESTONE_2_COMPLETION_REPORT.md` | Plan §8 criteria 1–13 |
| 4.2.2a | Write the Milestone 2 execution plan (scope · approved decisions relied on · RED gates · deliverables · pass/fail criteria) | ✅ Written 2026-09-21 | ADR-006 decided | ADR-006 | `MILESTONE_2_EXECUTION_PLAN.md` | Plan reviewed; **scope acceptance recorded separately from execution authorization** — *acceptance pending founder review* |
| 4.2.2b | Identity mapping pattern — our own immutable UUID as business identity; provider subjects confined to `auth_identities`; no business table references a provider ID | ✅ Built + tested 2026-09-21 | 4.2.2a | ADR-006 B3 condition 3 | `src/modules/identity/users.repository.ts`; `tests/integration/identity.test.ts` | Enforced in code and covered by tests |
| 4.2.2c | Authorization enforcement — `user_roles` in our PostgreSQL as the **only** authorization source of truth, with negative cases tested | ✅ Built + tested 2026-09-21 | 4.2.2a | ADR-006 B3 condition 2 | `src/modules/identity/{roles.repository,session}.ts`; `tests/e2e/identity.spec.ts` (403 / MFA gate) | Tier 1 workflow #2 covered end to end, negative cases included |
