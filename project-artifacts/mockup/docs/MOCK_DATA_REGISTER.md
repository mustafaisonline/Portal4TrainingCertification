...
| **Contact enquiry form** (`/contact`) | **Inert — and says so** | No backend exists, so the form submits nowhere. It shows a plain "Not connected yet" line rather than a fake success toast; a simulated "thanks, we'll be in touch" would be a prohibited mock success state |
| **Contact details** (`/contact`) | **REAL — with a stated gap** | Only genuine channels appear: the founder's LinkedIn, Medium, YouTube and Substack (exact URLs) plus the location from `data/practitioners.ts`. ⚠ **There is no business email, phone, office address or response-time commitment anywhere on the page, because none exists.** None may be invented — a wrong address silently loses real enquiries. Open item: see [`SITE_PAGES.md`](SITE_PAGES.md) |
| **`/about` positioning claims** | **REAL — traced, not written** | Every claim is taken from DR-02 §1/§2/§6/§7. Nothing is stated about scale, history, client names, team size, founding date, revenue or accreditation, because none of it is established in any approved source |
