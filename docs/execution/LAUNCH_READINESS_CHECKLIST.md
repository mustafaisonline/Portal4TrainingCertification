...
| C7 | `/programs/data-blueprint-ai-vibe-coding` (was `/DataBlueprint-AIVibeCoding`) | "24+ years … 40,000+ member Big Data community … 80+ episodes" | ~~code (hardcoded in the bespoke landing)~~ | **Removed 2026-09-26** — the bespoke landing was retired (the flagship renders on the shared training template); its trainer section is now the template's `TrainerCard`, which reads the seeded expert record. The "40,000+ … 80+ episodes" sentence no longer renders on any programme page; the figures that remain are C6's, from the seed |
...
| # | Route | Claim | Origin | Source / state |
|---|---|---|---|---|
| C10 | flagship page (`/programs/data-blueprint-ai-vibe-coding`), `/programs` | Malaysia "RM 4,999 / ~~RM 19,999~~ · 75% OFF · you save RM 15,000" + note "In-person training price. Minimum 25 participants. There is no online option for this training in Malaysia." | seed `courses.ts` | **Founder change list 2026-09-26 (second round):** today's figure RM 4,999 is the founder's; the ORIGINAL RM 19,999 was set by the orchestrator so that 75% is exact — **founder to confirm the original**. (History: 2026-09-06 RM 9,999 → RM 4,999, 50% OFF) |
| C10a | `/programs/learn-vibe-coding`, `/programs` | Learn Vibe Coding: Malaysia **RM 500** (~~RM 2,000~~) · Pakistan **Rs. 5,000** (~~Rs. 20,000~~) · International **USD 200** (~~USD 800~~), all "75% OFF", struck-through original + "you save" shown; note under Malaysia and International: "Online training price. In-person training needs a minimum of 25 participants; cost discussed separately." | seed `courses.ts` | **Founder-given figures 2026-09-26 (second round), implemented as written** — supersede the earlier RM 100 / Rs. 5,000 / USD 1,000. The founder typed "RM200" under USD; read as **USD 200 — founder to confirm** |
...
| C10c | `/programs/data-blueprint-ai-vibe-coding` | "Start freelancing or lead data-and-AI work straight after — you leave able to offer, scope and deliver work that combines trusted data with AI-built software" + six capability bullets (no income claim) | seed `courses.ts` (`afterThisTraining`) | **Founder-directed 2026-09-26** ("trainees will be able to start freelancing immediately"). Same review as C10b. Replaces the landing's C30 wording |
...
| C11 | same | Pakistan, two figures on one card: "In-person Rs. 199,999 / ~~Rs. 799,999~~ · minimum 100 participants" and "Online Rs. 99,999 / ~~Rs. 399,999~~ · minimum 10 participants" · 75% OFF. `programme_prices` holds ONE Pakistan row — the online figure (Rs. 399,999 → Rs. 99,999, PKR); the in-person figure is display-only in `content.regionalPricing` | seed `courses.ts` | **Founder change list 2026-09-26:** today's figures are the founder's; the originals (Rs. 799,999 / Rs. 399,999) were set by the orchestrator for exact 75% — **founder to confirm**. "Regional scholarship" framing retired |
| C11a | `/programs/*` Investment cards, `/checkout/[id]` | **Pakistan does not pay by card.** Malaysia pays by card in RM; everyone outside Malaysia and Pakistan pays by card in USD; Pakistan's card and the checkout screen show "Please contact us — our local partner will contact you to arrange payment through local banks or in cash." with a Contact us link. A fourth card, "Can't pay by card?", offers the same route anywhere in the world | code `PRICE_REGIONS.payment` (types.ts), `checkout.service.ts` (`card_payment_unavailable` — refused before any order or seat hold), `ProgrammePricing.tsx`, checkout page | **Founder-directed 2026-09-26.** Enforced server-side and tested (integration + e2e). Region detection unchanged (profile ISO country → `regionForCountry`) — **Verified** |
| C12 | same | International "USD 1,999 / ~~USD 7,999~~ · 75% OFF" + note "Online training price. In-person training needs a minimum of 100 participants; cost discussed separately." | seed | **Founder change list 2026-09-26:** USD 1,999 is the founder's; original USD 7,999 set by the orchestrator — **founder to confirm** |
| C13 | same | ~~Value stack "Total value RM 14,498+"~~ — **removed 2026-09-26** (founder: no value stack, no "Included" list on either published page) | seed / `ProgrammePricing.tsx` | Closed — nothing to verify |
...
| C15 | programme pricing block | ~~"Enquiry-based — no online payment yet"~~ — gone with the 2026-09-26 card rewrite; each card now states "How you pay: Card payment in RM / USD" or "Payment through our local partner", with "See upcoming dates" (→ `/schedule` → checkout) and "Register your interest" | code `ProgrammePricing.tsx` | Closed 2026-09-26 — the block no longer contradicts the live checkout |
...
| C23 | `/programs/[slug]` | ~~"This course awards a **certificate of participation**"~~ → "This course awards a **certificate of completion**" on both published pages | seed `courses.ts` (`certificate:` field) | **Changed 2026-09-26** — the flagship's seed now says "Certificate of Completion" (M6 decision E1), matching Learn Vibe Coding, `/verify` and the certificate itself. The five UNLISTED programmes still carry "Certificate of participation" in the seed; they render nowhere. Founder to confirm the wording stays when those are ever published |
...
| C27c | `/programs/[slug]` (shared `DeliveryFormats`; the flagship page is this template since 2026-09-26) | "All formats cover the same curriculum, learning outcomes, exercises and certification requirements" | code | **UNVERIFIED — founder to confirm**. Note the flagship's hero duration is now "2 days" (founder, 2026-09-26) while its Accelerator (2 weeks) and Mastery (4 weeks) format cards remain — founder to confirm the three formats still stand for a two-module, two-day training |
| C27d | `/programs/data-blueprint-ai-vibe-coding` | Curriculum "2 modules": Module 1 · Data Blueprint (the former modules 1–10 as grouped topics), Module 2 · Learn Vibe Coding (the Learn Vibe Coding curriculum verbatim, one shared constant) | seed `courses.ts` | **Founder-directed 2026-09-26.** The former modules 11–17 (Product discovery & validation, AI-assisted requirements engineering, Prompt engineering & PromptOS, Rapid application development, Quality engineering & testing, Deployment & production readiness, Capstone project) are **retired** — kept as a comment in the seed for reference. Copy that named PromptOS was removed with them |
...
| C28 | `/programs/data-blueprint-ai-vibe-coding` | ~~"a company's workflow that used to take 25 people now runs on 5"~~ | ~~code (bespoke landing)~~ | **Removed 2026-09-26** with the bespoke landing (the flagship renders on the shared template; the landing's hardcoded "benefits" block was not carried over). Renders nowhere now — the tension with C33 is gone. Founder may reinstate it in seed content once substantiated |
| C29 | same | ~~"exposed to 10–15 real frameworks"~~ | ~~code (bespoke landing)~~ | **Removed 2026-09-26** with the bespoke landing. Renders nowhere |
| C30 | same | ~~"Designed to lead to paid work"; "Freelance-ready from day one … real client work immediately"; "visibility with the practitioner network behind this Academy"~~ | ~~code (bespoke landing)~~ | **Removed 2026-09-26** with the bespoke landing ("Earn" journey stage, benefits block, "Stop prompting" CTA). The freelancing promise is now the seeded, capability-only C10c block — review that instead |
...
| C33 | schedule, trainers, about, reviews | Honest empty states: "The first dates are being prepared"; "no placeholder profiles … and there never will be"; "No reviews have been published yet"; "No invented dates, cohort numbers, client logos or testimonials" | code / DB | DR-02 §4.1, §7 — **Verified**. The programme page's "What participants say — the first cohort has not run yet … we will not invent any" placeholder went with the bespoke landing (2026-09-26); `/reviews` is the one place published reviews appear, and it keeps its own empty state. C28 is removed, so the tension noted here before is gone |

### 2.9 Summary for the founder

Seven decisions clear most of the table: **(1)** supply the legal-entity details to counsel (C26/B1); **(2)** back or remove the "brought to Malaysia" promise (C27); **(3)** ~~substantiate or remove the 25→5 claim (C28)~~ — removed with the bespoke landing 2026-09-26; **(4)** confirm the three price rows and set an offer end date (C10–C14); **(5)** fix the three stale payment sentences (C15, C16, C26b); **(6)** ~~pick "completion" vs "participation" (C23)~~ — "Certificate of Completion" applied to the flagship 2026-09-26; **(7)** confirm the trainer figures (C6; C7 no longer renders) and format durations (C27b, and now C27c's two-day vs Accelerator/Mastery question). Everything else is verified or a wording check for counsel.
...
|---|---|---|
| `/DataBlueprint-AIVibeCoding` | `/programs/data-blueprint-ai-vibe-coding` (the flagship's page — since 2026-09-26 the shared training template; the bespoke landing was retired, `FlagshipLanding.tsx` removed, history keeps it) | **Configured**, asserted in `tests/e2e/trainings.spec.ts` |
