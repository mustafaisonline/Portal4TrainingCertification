...
describe("database timestamps", () => {
  it("the session timezone is UTC", async () => {
    const [{ TimeZone }] = await getPrisma().$queryRaw<{ TimeZone: string }[]>`SHOW timezone`;
  ...
  it("a Prisma-written timestamp equals the database clock (no zone skew)", async () => {
    ...
    try {
      ...
      expect(Math.abs(Number(skew)), `skew of ${skew}s between DB now() and created_at`).toBeLessThan(60);
