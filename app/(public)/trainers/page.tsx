 ...
 * Changed: trainers read from `listPublishedExperts()` (`photoPath`/`roleTitle`/
 * `profile.linkedin`/`profile.mediumProfile` replace the mockup's flat fields),
 * guarded for an empty list; the accreditation's "Valid from / to" now render
 * the record's own `validFrom`/`validTo` dates instead of two hardcoded
 * strings; the "Community" YouTube section (`CommunityVideos`, `lib/youtube.ts`)
 * is NOT ported — its browser-side API key is an open decision (ADR-030) —
 * and nothing stands in its place; `assetPath()` dropped; the mockup's own
...
/** ISO date (YYYY-MM-DD) → "8 July 2026", as the mockup displayed it. */
...
export default async function TrainersPage() {
  const practitioners = await listPublishedExperts();
  const [lead, ...others] = practitioners;
  const accreditation = lead?.hrdCorpAccreditation ?? null;
  const [leadExperienceHeadline] = lead ? lead.experienceLine.split(" · ") : [];
  const [leadCity, leadDeliveryNote] = lead ? lead.location.split(" · ") : [];

  return (
    <>
      {/* ===== Hero ===== */}
      <section className="night hero-band relative">
            ...
            >
              <div className="mb-5 flex flex-col gap-4 min-[420px]:flex-row min-[420px]:gap-5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-label mb-2">{lead.roleTitle}</p>
              ...
              >
                <div className="mb-5 flex flex-col gap-4 min-[420px]:flex-row min-[420px]:gap-5">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-label mb-2">{person.roleTitle}</p>
                    <div className="flex flex-col gap-1.5 text-body-sm text-[var(--color-ink-quiet)]">
                      <span className="flex items-center gap-2">
              ...
              <div>
                ...
                <dl className="mb-8 grid grid-cols-2 gap-x-8 gap-y-5 border-t border-[var(--color-line)] pt-7 sm:grid-cols-3">
                  {[
                    ...
                    ["Valid from", formatDate(accreditation.validFrom)],
                    ["Valid to", formatDate(accreditation.validTo)],
                  ].map(([label, value]) => (
                    <div key={label}>
      ...
      {/* ===== Community ("From the show") — NOT PORTED, see file header.
          Re-evaluate server-side (ADR-030) before adding it back. */}
    </>
  );
}
