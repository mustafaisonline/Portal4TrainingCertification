...
| `data/demoParticipant.ts` | The participant, the **3 invented start dates**, generic payment methods, all currencies to everyone | Real records; decided list (A9); eligibility rule (A8) · **✅ The invented offerings/`sampleSeats` retired by M3 (2026-09-21): `scheduled_offerings` table, zero rows, honest "no dates yet" state on `/schedule`.** Participant/payment parts wait for M4/M5 |
| `data/courses.ts`, `practitioners.ts`, `domains.ts`, `faq.ts`, `questions.ts` | Reviewed content held as TypeScript constants | **✅ Retired by M3 (2026-09-21)** — ported verbatim to `prisma/seed-data/` and seeded into `programmes` (+ modules, delivery formats, prices), `experts`, `domains`, `faq_entries`, `diagnostic_questions`; pages read repositories |
| `data/results.ts`, `data/roles.ts` (diagnostic result fixtures) | **Two canned, invented result profiles** | **✅ Not ported (M3)** — `/diagnostic/result` shows an honest per-area answer summary until an assessment engine exists |
