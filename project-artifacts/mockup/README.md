...
| `/faq`, `/schedule`, `/for-organisations`, `/terms`, `/privacy`, `/refund-policy`, `/credential-integrity-policy` | Added 2026-09-20 (review round). FAQ with honest "to be confirmed" answers; sample schedule with seats/waitlist; team engagement page; four **placeholder** legal pages (no policy text) |
| `/account/orders/[id]`, `/account/notifications`, `/account/help`, `/admin/*` | Added 2026-09-20. Sample receipt; notification samples; help signposts; **trainer/admin wireframe** — 15 screens under `/admin`, second demo persona `demo.admin@example.com` / `Demo-Admin-2026` (sample tables, disabled actions; the role is a browser label, not a permission) |
