...
| Duplicate registration (plan §6.6 enumeration-safety) | **Option A — a clear error.** Founder found that a repeat sign-up returned "account created" while the original password silently remained (Better Auth's synthetic success). Now `/sign-up/email` answers 422 `USER_ALREADY_EXISTS` ("An account with this email already exists. Sign in, or reset your password.") and never touches the existing credential | `auth.ts` before-hook; register form message |
| Admin two-factor authentication (§9.2, OQ-14 default "Yes") | **Removed for MVP 1** (founder, 2026-09-21). The `twoFactor` plugin, enrolment page, second sign-in step and the MFA condition in `authorise()` are gone; `platform_admin` alone opens `/admin`. `auth_two_factors` and `auth_users.two_factor_enabled` stay in the schema unused (dropping them is a destructive migration for separate approval). Reinstating = restoring the wiring from commit `5d08cdc`. Criteria 8 (MFA branch) and 9 are superseded | `auth.ts`, `auth-client.ts`, `session.ts`, admin layout; MFA routes and TOTP test helper deleted |
| Change password while signed in | **Added** at `/account/security` (current password required; other sessions revoked; audited as `password.changed`) — "forgot password" cannot deliver a link until ADR-015 is decided | `app/account/security/*`; `auth.ts` after-hook |
