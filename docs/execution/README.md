...
| [`MILESTONE_1_EXECUTION_PLAN.md`](MILESTONE_1_EXECUTION_PLAN.md) | The "Walking Skeleton" — the proposed first executable milestone: scope, approved decisions relied on, RED-gate actions, deliverables, verification criteria. **Unaffected by the `DR-02` correction** | ✅ **EXECUTED 2026-09-21** — [`MILESTONE_1_COMPLETION_REPORT.md`](MILESTONE_1_COMPLETION_REPORT.md) |
...
| [`MILESTONE_6_EXECUTION_PLAN.md`](MILESTONE_6_EXECUTION_PLAN.md) | **Certificate of Completion & verification** — admin-recorded completion → idempotent issuance (unique ID), holder page with the reviews gate on the document, public `/verify` search and `/verify/[id]`, renewal by Stripe with an effective-dated fee, revocation, listing consent; 14 decisions (§3) and the Rule 1 schema (§4) for the founder | ✅ **EXECUTED 2026-09-22** — [`MILESTONE_6_COMPLETION_REPORT.md`](MILESTONE_6_COMPLETION_REPORT.md) |
| [`MILESTONE_7_EXECUTION_PLAN.md`](MILESTONE_7_EXECUTION_PLAN.md) | **Renewal reminders** — 30/7-day and lapsed reminders to the outbox, idempotent through audit rows, authenticated jobs endpoint for whatever scheduler hosting provides, holder notifications, admin "last run"; no schema change | ✅ **EXECUTED 2026-09-23** — [`MILESTONE_7_COMPLETION_REPORT.md`](MILESTONE_7_COMPLETION_REPORT.md) |
| [`MILESTONE_8_EXECUTION_PLAN.md`](MILESTONE_8_EXECUTION_PLAN.md) | **Trainer / admin operations** — admin navigation, orders & payments (read-only), enquiries, users & roles, audit log, reports with CSV, participant data export; §5 lists organisations, invoices, attendance, refund-on-behalf, account deletion **for founder approval (schema / policy)** | ✅ **EXECUTED 2026-09-23** (existing-schema scope) · ⛔ §5 awaiting approval — [`MILESTONE_8_COMPLETION_REPORT.md`](MILESTONE_8_COMPLETION_REPORT.md) |
| [`MILESTONE_9_EXECUTION_PLAN.md`](MILESTONE_9_EXECUTION_PLAN.md) | **Production readiness** — health endpoint, security headers, fail-fast config, error pages, robots/sitemap, backup & restore scripts, Dockerfile, runbooks, restart-resilience test; §4 lists hosting, DB host, residency, domain, email provider, release gate **for the founder** | ✅ **EXECUTED 2026-09-23** (code + runbooks) · ⛔ §4 decisions and provisioning are the founder's — [`MILESTONE_9_COMPLETION_REPORT.md`](MILESTONE_9_COMPLETION_REPORT.md) · [`../operations/`](../operations/README.md) |
| [`MILESTONE_10_EXECUTION_PLAN.md`](MILESTONE_10_EXECUTION_PLAN.md) | **Launch cutover** — readiness checklist, claims reconciliation, wireframe-remnant check; the cutover itself (legal publication, live Stripe, domain, mockup archive) is the founder's | ✅ **AUDIT COMPLETE 2026-09-23** · ⛔ cutover is the founder's — [`MILESTONE_10_COMPLETION_REPORT.md`](MILESTONE_10_COMPLETION_REPORT.md) · [`LAUNCH_READINESS_CHECKLIST.md`](LAUNCH_READINESS_CHECKLIST.md) |
| [`MILESTONE_11_EXECUTION_PLAN.md`](MILESTONE_11_EXECUTION_PLAN.md) | **DigitalOcean deployment framework & Stripe go-live** — founder request 2026-09-26. Corrects the premise (Stripe is already built, M4); compares the portal's gateway with eCard's; the one real gap (restricted `rk_` keys are rejected by start-up validation); a `deploy/` framework porting eCard's *controls* (signed token, root wrapper, backup-before-deploy, sandbox migration, rollback, audit, reports) onto the portal's own runtime model (CI-built image, managed PostgreSQL, Caddy); three phases; costed; **16 numbered decisions (§5)**; opens with the live-key rotation incident (§0) | ✅ **K1–K16 APPROVED 2026-09-26 (ADR-046) · Phase A BUILT 2026-09-26** (§8: `env.ts` accepts `rk_` keys; `deploy/` framework dry-run-proven; `release.yml` written, **not yet run**) · ⛔ **Phase B (provisioning) and C are the founder's — nothing exists yet** — [`../../deploy/README.md`](../../deploy/README.md) |
| [`MILESTONE_12_EXECUTION_PLAN.md`](MILESTONE_12_EXECUTION_PLAN.md) | **Trainer role & training management** — founder request 2026-09-26: admin grants a Trainer role; trainings, schedule and a **four-row fee structure** (Malaysia via HRD Corp · Malaysia not via HRD Corp · Pakistan · Rest of the world) managed in the portal, in the database, never in seed files or front-end code. What exists (schedule done; fees need a schema change; trainings read-only today), four work packages, **three additive schema changes (§4)**, **12 decisions (§6)** | ✅ **L1–L12 APPROVED (ADR-047) · BUILT 2026-09-26** — §8: four-row fees (migration), `/admin/trainings` with six tabs, Trainer role, launch cards, dates + Register on the training page, `/schedule` for every training; Vitest 438/438 |
